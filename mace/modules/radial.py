...
class GaussianBasis(torch.nn.Module):
    ...
    def __init__(self, num_basis=32):
        super().__init__()
        gaussian_weights = torch.linspace(
            start=0.0, end=5, steps=num_basis, dtype=torch.get_default_dtype()
        )
        coeff_weights = torch.ones(gaussian_weights.shape, dtype=torch.get_default_dtype())
        self.gaussian_weights = torch.nn.Parameter(
                gaussian_weights, requires_grad=True
            )
        self.coeff = torch.nn.Parameter(
                coeff_weights, requires_grad=True
            )

    def forward(self, x: torch.Tensor) -> torch.Tensor:  # [..., 1]
        x = x - self.gaussian_weights
        return torch.mean(torch.exp(-self.coeff * torch.pow(x, 2)), dim=-1)
