...
class FranckCondonMACE(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
        ...
        self.readouts = torch.nn.ModuleList()
        if n_vectors > 0:
            self.readouts.append(LinearDipoleReadoutBlock(hidden_irreps, n_energies, n_scalars, n_vectors))
        else:
            self.readouts.append(LinearReadoutBlock(hidden_irreps, n_energies, n_scalars, n_vectors))
        ...
        for i in range(num_interactions - 1):
            ...
            if i == num_interactions - 2:
                if n_vectors > 0:
                    self.readouts.append(NonLinearDipoleReadoutBlock(hidden_irreps_out, MLP_irreps, gate, n_energies, n_scalars, n_vectors))
                else:
                    self.readouts.append(NonLinearReadoutBlock(hidden_irreps_out, MLP_irreps, gate, n_energies, n_scalars))
            else:
                if n_vectors > 0:
                    self.readouts.append(LinearDipoleReadoutBlock(hidden_irreps, n_energies, n_scalars, n_vectors))
                else:
                    self.readouts.append(LinearReadoutBlock(hidden_irreps, n_energies, n_scalars))
    ...
    def forward(
        ...
        for interaction, product, readout, field_interaction in zip(
            ...
            node_feats_list.append(node_feats)
            if self.n_energies + self.n_scalars == 1:
            ...
            node_scalars = node_output[:, self.n_energies:self.n_energies + self.n_scalars]
            
            node_vectors = node_output[:, self.n_energies + self.n_scalars:]
            node_vectors_list.append(node_vectors.reshape(node_vectors.shape[0], self.n_vectors, 3))

            energy = scatter_sum(
                src=node_energies, index=data["batch"], dim=-1, dim_size=num_graphs
            )  # [n_graphs,]
            vectors = scatter_sum(
                src=torch.transpose(node_vectors, 0, 1), index=data["batch"], dim=1, dim_size=num_graphs
            )
            vectors_list.append(torch.transpose(vectors, 0, 1))
            node_scalars_list.append(node_scalars)
        ...
        contributions = torch.stack(energies, dim=1)
        total_energy = torch.sum(contributions, dim=1)  # [n_graphs, ]
        vector_contributions = torch.stack(vectors_list, dim=1)
        total_vectors = torch.sum(vector_contributions, dim=1)
        total_vectors = total_vectors.reshape(total_vectors.shape[0], self.n_vectors, 3)
        node_scalars_contributions = torch.stack(node_scalars_list, dim=1)
        total_node_scalars = torch.sum(node_scalars_contributions, dim=1)        
        node_energy_contributions = torch.stack(node_energies_list, dim=1)
        node_energy = torch.sum(node_energy_contributions, dim=1)  # [n_nodes, ]
