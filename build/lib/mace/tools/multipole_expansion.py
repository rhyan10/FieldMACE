    ...
    def _multipole_contribs(self, xyz):
        ...
        for l in range(self.l_max + 1):
            for m in range(-l, l + 1):
                q_lm = self.multipole_moments[(l, m)]
                if self.exterior:
                    phi_l = np.sqrt(4 * np.pi / (2 * l + 1)) * q_lm / r ** (l + 1)
                else:
                    phi_l = np.sqrt(4 * np.pi / (2 * l + 1)) * q_lm * r ** l
