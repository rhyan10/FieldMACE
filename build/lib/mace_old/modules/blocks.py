...
#     def __init__(self, irreps_in, irreps_hidden, irreps_out, num_random_features=64):
...
#         self.to_keys = o3.Linear(self.irreps_in, self.irreps_hidden)
#         self.to_values = o3.Linear(self.irreps_in, self.irreps_hidden)
        
#         self.num_random_features = num_random_features
#         self.register_buffer('omega', torch.randn(self.irreps_hidden.dim, num_random_features))
#         self.register_buffer('bias', 2 * np.pi * torch.rand(num_random_features))
        
#         # Output mapping
#         self.to_out = o3.Linear(self.irreps_hidden, self.irreps_out)
    
#     def kernel_feature_map(self, x):
#         # x: (batch_size, sequence_length, irreps_hidden.dim)
#         # Compute the random features in a way that preserves equivariance
#         x_omega = torch.einsum('bld,df->blf', x, self.omega)  # (batch_size, sequence_length, num_random_features)
#         feature_map = torch.cos(x_omega + self.bias) * np.sqrt(2.0 / self.num_random_features)
#         return feature_map  # (batch_size, sequence_length, num_random_features)
    
#     def forward(self, x):
#         batch_size, sequence_length, _ = x.shape
        
#         queries = self.to_queries(x)  # (batch_size, sequence_length, irreps_hidden.dim)
#         keys = self.to_keys(x)        # (batch_size, sequence_length, irreps_hidden.dim)
#         values = self.to_values(x)    # (batch_size, sequence_length, irreps_hidden.dim)
#         # Compute kernel feature maps
#         query_features = self.kernel_feature_map(queries)  # (batch_size, sequence_length, num_random_features)
#         key_features = self.kernel_feature_map(keys)       # (batch_size, sequence_length, num_random_features)
        
#         # Compute approximate attention scores
#         # Attention numerator: (batch_size, sequence_length, num_random_features)
#         numerator = torch.einsum('blf,bld->blf', query_features, values)
        
#         # Attention denominator: (batch_size, sequence_length, num_random_features)
#         denominator = torch.einsum('blf,bkf->blk', query_features, key_features).sum(dim=-1, keepdim=True) + 1e-6
        
#         # Compute attention output
#         context = torch.einsum('blk,bkd->bld', query_features, torch.einsum('blk,bkf->blf', key_features.transpose(1, 2), values))
#         context = context / denominator
        
#         # Map to output
#         output = self.to_out(context)  # (batch_size, sequence_length, irreps_out.dim)
        
#         # Aggregate over the sequence length if needed
...
class RotInvariantAttention(torch.nn.Module):
    ...
    def forward(self, x, node_feats):
        # x: (batch_size, sequence_length, irreps_in.dim)
        batch_size, sequence_length, _ = x.shape

        queries = self.to_queries(x)  # (batch_size, sequence_length, irreps_hidden.dim)
        keys = self.to_keys(node_feats)        # (batch_size, sequence_length, irreps_hidden.dim)
        values = self.to_values(x)    # (batch_size, sequence_length, irreps_hidden.dim)
        ...
        queries_expanded = queries.unsqueeze(2)  # (batch_size, sequence_length, 1, irreps_hidden.dim)
        keys_expanded = keys.unsqueeze(1)        # (batch_size, 1, sequence_length, irreps_hidden.dim)

        diff = queries_expanded-keys_expanded

        scores = torch.squeeze(self.scores_multi(diff), dim=-1)

        # Remove the last dimension by reshaping instead of squeezing
        scores = torch.transpose(scores, 1, 2)

        # Compute attention weights
        attention_weights = torch.softmax(scores / scores.size(-1)**0.5, dim=-1)  # Along key dimension
...
class ElectrostaticFieldInteractionBlock(torch.nn.Module):
    def __init__(
        ...
        # print(self.node_feats_irreps)
        # print(self.field_irreps)
        # print(irreps_mid)
        # print("!!!!!!!!!!!!!!!!!!!!!!!")
        self.node_feats_irreps = Irreps("1x0e + 1x1o + 1x2e + 1x3o")
        self.field_irreps = Irreps("1x0e + 1x1o + 1x2e + 1x3o")
        #irreps_mid = self.node_feats_irreps.tp(self.field_irreps)
        #print(self.node_feats_irreps)
        ...
        # print(self.conv_tp)
        # self.conv_tp.visualize()
        # Convolution weights
        input_dim = field_irreps.num_irreps
        self.conv_tp_weights = nn.FullyConnectedNet(
            [input_dim] + [128, 128, 128] + [self.conv_tp.weight_numel],
            torch.nn.functional.silu,
        )
        # Linear
        irreps_mid = "4x0e+6x1o+3x1e+4x2o+7x2e+6x3o+4x3e+2x4o+4x4e+2x5o+1x5e+1x6e"
        #self.irreps_out = self.target_irreps
        self.irreps_out = Irreps("1x0e + 1x1o + 1x2e + 1x3o")
        self.linear = o3.Linear(
            irreps_mid, self.irreps_out, internal_weights=True, shared_weights=True
        )
        self.reshape = reshape_irreps(self.irreps_out)
    ...
    def forward(
        ...
        multipole_attrs: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        # print(self.linear_up)
        # print(self.conv_tp_weights)
        # print(self.conv_tp)
        #node_feats = self.linear_up(node_feats)
        #tp_weights = multipole_feats
        multipole_feats = multipole_feats.unsqueeze(1)
#        multipole_feats = multipole_feats.repeat(1, 128, 1)
        # print(node_feats.shape)
        # print(multipole_feats.shape)
        # print("################")
        mi = self.conv_tp(
            node_feats, multipole_feats#, tp_weights.unsqueeze(1),
        )
#        print(mi.shape)
        #print(mi.irreps)
        field_message = self.linear(mi)
#        print(field_message.shape)
