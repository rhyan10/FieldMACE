    ...
    def forward(
        ...
        for interaction, product, readout, field_interaction in zip(
            ...
            field_feats = field_interaction(
                node_feats=intermed_node_feats,
                multipole_feats=multipole_feats,
                multipole_attrs=multipole_attrs,
            )
        ...
        if self.compute_dipoles:
            dipole_contributions = torch.stack(node_dipoles_list, dim=1)
            total_dipoles = torch.sum(dipole_contributions, dim=1)  # [n_graphs, ]
            shape = total_dipoles.shape[0]
            #print(shape)
            total_dipoles = total_dipoles.reshape(int(shape/12), 12, 1, 3)
            total_dipoles = torch.sum(total_dipoles, dim=1)
...
class AttentionFieldEMACE(torch.nn.Module):
    ...
    def forward(
        ...
        multipoles = compute_multipole_expansion(positions_internal=data["positions"], multipoles=attention_aggregated_moments, batch=data["batch"], lmax=self.lmax)
        multipole_feats = self.multipole_radial_embedding(multipoles).squeeze()
        pair_node_energy = torch.zeros_like(node_e0)
        pair_energy = torch.zeros_like(e0)
        # Interactions
        ...
        if self.compute_dipoles:
            ...
            total_dipoles = torch.sum(dipole_contributions, dim=1)  # [n_graphs, ]
            shape = total_dipoles.shape[0]
            total_dipoles = total_dipoles.reshape(int(shape/50), 50, 1, 3)
            total_dipoles = torch.sum(total_dipoles, dim=1)
